...
export default class Texture3D {
  constructor(context, width, height, depth, format, type) {
      index++;
    this.gl = context;
  ...
  upload(image,width,height,depth) {
    ...
    this.gl.texImage3D(this.gl.TEXTURE_3D, 0, this._internalformat, image.width, image.height, depth, 0, this._format, this._type, image);
  ...
  uploadData(data, width, height, depth) {
    ...
    if (data instanceof Float32Array) {
      ...
      this.type = this.gl.FLOAT;
    		...
    		// gl.texParameteri(gl.TEXTURE_3D, gl.TEXTURE_BASE_LEVEL, 2);
      this.gl.texImage3D(this.gl.TEXTURE_3D, 0, this._internalformat, width, height, depth, 0, this._format, this._type, data || null);
